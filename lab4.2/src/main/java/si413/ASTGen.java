...
public class ASTGen {
    ...
    private class StmtVisitor extends Visitor<Stmt> {
        ...
        { return new Stmt.Assign(ctx.ID().getText(), eVis.visit(ctx.funcEx()));}

        @Override
        public Stmt visitDefStrFun(ParseRules.DefStrFunContext ctx)
        {
            return new Stmt.FuncDef(ctx.ID().getText(), plVis.visit(ctx.paramList()), stlVis.visit(ctx.stmtList()), eVis.visit(ctx.strEx()));
		}
        @Override
        public Stmt visitDefBoolFun(ParseRules.DefBoolFunContext ctx)
        { 
            return new Stmt.FuncDef(ctx.ID().getText(), plVis.visit(ctx.paramList()), stlVis.visit(ctx.stmtList()), eVis.visit(ctx.boolEx()));
		}
        @Override
        public Stmt visitDefVoidFun(ParseRules.DefVoidFunContext ctx)
        {
            return new Stmt.VoidFuncDef(ctx.ID().getText(), plVis.visit(ctx.paramList()), stlVis.visit(ctx.stmtList()));
        }
        @Override
        public Stmt visitDefFunFun(ParseRules.DefFunFunContext ctx)
        {
            return new Stmt.FuncDef(ctx.ID().getText(), plVis.visit(ctx.paramList()), stlVis.visit(ctx.stmtList()), eVis.visit(ctx.funcEx()));
		}
        @Override
        public Stmt visitVoidFunCall(ParseRules.VoidFunCallContext ctx)
        {
			//thinking this: return arguments to the exev stmt which creates the new frame and adds parameters to it. The frame is executed when the funcExpr is evaluated in the main Frames list of binds: (fname, fcode)
            return new Stmt.VoidFunCall(eVis.visit(ctx.funcEx()), alVis.visit(ctx.argList()));
        }
	...
	{
		public Expr visitStrFunCall(ParseRules.StrFunCallContext ctx)
		{
            return new Expr.FunCall(eVis.visit(ctx.funcEx()), alVis.visit(ctx.argList()));
		}

		public Expr visitBoolFunCall(ParseRules.BoolFunCallContext ctx)
		{
            return new Expr.FunCall(eVis.visit(ctx.funcEx()), alVis.visit(ctx.argList()));
		}

		public Expr visitFunFunCall(ParseRules.FunFunCallContext ctx)
		{
            return new Expr.FunFunCall(eVis.visit(ctx.funcEx()), alVis.visit(ctx.argList()));
		}
		...
		{
			return new Expr.FuncVar(ctx.ID().getText());
		}

 	}
	private ArgListVisitor alVis = new ArgListVisitor();
	private ArgVisitor aVis = new ArgVisitor();
	private ParamListVisitor plVis = new ParamListVisitor();
	private ParamVisitor pVis = new ParamVisitor();
